...
public class TimeAfterInputReader<T> implements TaxiReader<T> {
  ...
  private final Class<T> target;
    ...
    public TimeAfterInputReader(String searchedTimeColumnName, LocalDateTime searchedTimeValue, TaxiReader<T> decorated, Class<T> target) {
  ...
  public Dataset<T> readInput(SparkSession session) {
    return decorated.readInput(session)
        .where(functions.col(searchedTimeColumnName).gt(Timestamp.valueOf(searchedTimeValue)))
